angular.module('seroApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('partials/advancedCodes.html',
    "<div class=top-header><h1>Codes Management</h1></div><div class=advanced><div class=row><div class=col-md-8></div><div class=col-md-4><button type=button class=\"btn btn-success pull-right\" ng-click=addRow()>New code</button></div></div><div class=row><div class=col-md-12><hr></div></div><div ui-grid=gridOptions ui-grid-edit ui-grid-row-edit ui-grid-cellnav ui-grid-resize-columns ui-grid-move-columns class=grid></div></div>"
  );


  $templateCache.put('partials/advancedEmailOrder.html',
    "<div class=top-header><h1>Email Order Management</h1></div><div class=advanced><div class=row><div class=col-md-8></div><div class=col-md-4><button type=button class=\"btn btn-success pull-right\" ng-show=gridOptions.data.length ng-click=exportEmailOrders(gridOptions.data)>Export to excel</button></div></div><div class=row><div class=col-md-12><hr></div></div><div ui-grid=gridOptions ui-grid-edit ui-grid-row-edit ui-grid-cellnav ui-grid-resize-columns ui-grid-move-columns class=grid></div></div>"
  );


  $templateCache.put('partials/advancedLeadAgency.html',
    "<div class=top-header><h1>Lead Agency Management</h1></div><div class=advanced><div class=row><div class=col-md-8></div><div class=col-md-4><button type=button class=\"btn btn-success pull-right\" ng-click=addRow()>New code</button></div></div><div class=row><div class=col-md-12><hr></div></div><div ui-grid=gridOptions ui-grid-edit ui-grid-row-edit ui-grid-cellnav ui-grid-resize-columns ui-grid-move-columns class=grid></div></div>"
  );


  $templateCache.put('partials/advancedUsers.html',
    "<div class=top-header><h1>User Management</h1></div><div class=advanced><div class=row><div class=col-md-8><div class=\"alert alert-warning\"><b>NB:</b><li>Users wth <b>Is Passive</b> set to <b>1</b> will not be able to login.</li><li>Users with <b>:non_essential</b> appended to their <b>Job Position Code</b> will be able to login, but not show up in drop-down lists.</li></div></div><div class=col-md-4><button type=button class=\"btn btn-success pull-right\" ng-click=addRow()>New user</button></div></div><div class=row><div class=col-md-12><hr></div></div><div ui-grid=gridOptions ui-grid-edit ui-grid-row-edit ui-grid-cellnav ui-grid-resize-columns ui-grid-move-columns class=grid></div></div>"
  );


  $templateCache.put('partials/anonymize-data.html',
    "<div class=top-header><h1>Anonymize Data</h1></div><div><p>Anonymize field data <sub style=color:#ff0000 ng-show=\"envinfo != TEST\">(Available only on Test)</sub></p><br><br><div class=\"alert alert-warning text-center\" ng-show=working>Processing...</div><div class=\"alert alert-success text-center\" ng-show=\"!working && successState\">Done</div><div class=\"alert alert-danger text-center\" ng-show=\"done == 2 && !working && !successState\">Failed :(</div><div class=row><div class=col-md-6><label>Organization emails</label></div><div class=col-md-6><button class=\"btn btn-warning\" ng-hide=\"working || envinfo != TEST\" ng-click='anonymize(\"org_email_update\")'>{{defaultBtnTxt}}</button></div></div><br><div class=row><div class=col-md-6><label>Practitioner emails</label></div><div class=col-md-6><button class=\"btn btn-warning\" ng-hide=\"working || envinfo != TEST\" ng-click='anonymize(\"prac_email_update\")'>{{defaultBtnTxt}}</button></div></div><br><div class=row><div class=col-md-6><label>EIA email contacts</label></div><div class=col-md-6><button class=\"btn btn-warning\" ng-hide=\"working || envinfo != TEST\" ng-click='anonymize(\"eia_email_update\")'>{{defaultBtnTxt}}</button></div></div><br><div class=row><div class=col-md-6><label>External Audit email contacts</label></div><div class=col-md-6><button class=\"btn btn-warning\" ng-click='anonymize(\"ea_email_update\")' ng-hide=\"working || envinfo != TEST\">{{defaultBtnTxt}}</button></div></div><br><div class=row><div class=col-md-6><label>Permit License email contacts</label></div><div class=col-md-6><button class=\"btn btn-warning\" ng-hide=\"working || envinfo != TEST\" ng-click='anonymize(\"pl_email_update\")'>{{defaultBtnTxt}}</button></div></div></div>"
  );


  $templateCache.put('partials/auditInspection-pirking.html',
    "<div class=top-header><h1>AI Status Update</h1></div><div class=pirking><p>Currently last ID used for a AI is {{lastId}}.</p><form role=form name=criteriaForm novalidation ng-submit=beginPirking()><div class=row><div class=col-md-2><label for=criteria.from>From ID (inclusive)</label> <input integer id=criteria.from ng-required=true ng-model=criteria.from class=\"form-control input-sm\"></div><div class=col-md-2><label for=criteria.to>To ID (inclusive)</label> <input integer id=criteria.to ng-required=true ng-model=criteria.to class=\"form-control input-sm\"></div><div class=col-md-1><label for=dryrun>Dry run</label> <input type=checkbox class=\"form-control input-sm pull-left checkbox-small\" id=dryrun ng-checked=dryrun ng-click=\"dryrun = !dryrun;\"></div><div class=col-md-7><label for=criteria.begin>&nbsp;</label><br><button type=submit class=\"btn btn-sm btn-primary\" id=criteria.begin ng-disabled=\"criteriaForm.$invalid || working\">Begin Status Update</button></div></div><div class=row><div class=col-md-12><br><span class=\"label label-info\" ng-show=dryrun>Try without saving the status back to the database.</span> <span class=\"label label-danger\" ng-hide=dryrun>This will save any changes back to the database.</span></div></div><div class=row><div class=col-md-12><br><span style=padding:5px; class=\"label label-info\">Progress <span class=badge>{{info.finished()}}/{{aiData.length}}</span></span> <span style=padding:5px; class=\"label label-success\">Changed <span class=badge>{{info.changed}}</span></span> <span style=padding:5px; class=\"label label-warning\">No change <span class=badge>{{info.nochange}}</span></span> <span style=padding:5px; class=\"label label-danger\">Error <span class=badge>{{info.error}}</span></span></div></div><div class=row><br><div class=col-md-12><div class=progress><div class=progress-bar role=progressbar style=\"width: {{info.progress()}}%;\"></div></div></div></div></form><table class=\"table table-condensed\"><thead><tr><th>AI ID</th><th>Project ID</th><th style=width:400px;>Project title</th><th colspan=2>Existing status</th><th colspan=2>New status</th><th>Updated at</th><th>Result</th></tr></thead><tbody><tr data-ng-repeat=\"ai in aiData\" ng-class=\"{danger: ai.error, warning: ai.nochange, success: ai.changed}\"><td>{{ai.auditsInspections_id}}</td><td>{{ai.project_id}}</td><td>{{ai.project_title}}</td><td>{{ai.status_id}}</td><td style=\"white-space: nowrap;\">{{ai.status_description}}</td><td>{{ai.status_id_new}}</td><td style=\"white-space: nowrap;\">{{ai.status_description_new}}</td><td style=\"white-space: nowrap;\">{{ai.externalaudit_updated_at | date:'yyyy-MM-dd HH:mm:ss'}}</td><td style=\"white-space: nowrap;\"><span ng-show=ai.updating class=\"loader pull-right\"></span>{{ai.result}}</td></tr></tbody></table></div>"
  );
  ...
  $templateCache.put('partials/eiapermit-pirking.html',
    "<div class=top-header><h1>EIA Status Update</h1></div><div class=pirking><p>Currently last ID used for a EIA is {{lastId}}.</p><form role=form name=criteriaForm novalidation ng-submit=beginPirking()><div class=row><div class=col-md-2><label for=criteria.from>From ID (inclusive)</label> <input integer id=criteria.from ng-required=true ng-model=criteria.from class=\"form-control input-sm\"></div><div class=col-md-2><label for=criteria.to>To ID (inclusive)</label> <input integer id=criteria.to ng-required=true ng-model=criteria.to class=\"form-control input-sm\"></div><div class=col-md-1><label for=dryrun>Dry run</label> <input type=checkbox class=\"form-control input-sm pull-left checkbox-small\" id=dryrun ng-checked=dryrun ng-click=\"dryrun = !dryrun;\"></div><div class=col-md-7><label for=criteria.begin>&nbsp;</label><br><button type=submit class=\"btn btn-sm btn-primary\" id=criteria.begin ng-disabled=\"criteriaForm.$invalid || working\">Begin Status Update</button></div></div><div class=row><div class=col-md-12><br><span class=\"label label-info\" ng-show=dryrun>Try without saving the status back to the database.</span> <span class=\"label label-danger\" ng-hide=dryrun>This will save any changes back to the database.</span></div></div><div class=row><div class=col-md-12><br><span style=padding:5px; class=\"label label-info\">Progress <span class=badge>{{info.finished()}}/{{eiaspermits.length}}</span></span> <span style=padding:5px; class=\"label label-success\">Changed <span class=badge>{{info.changed}}</span></span> <span style=padding:5px; class=\"label label-warning\">No change <span class=badge>{{info.nochange}}</span></span> <span style=padding:5px; class=\"label label-danger\">Error <span class=badge>{{info.error}}</span></span></div></div><div class=row><br><div class=col-md-12><div class=progress><div class=progress-bar role=progressbar style=\"width: {{info.progress()}}%;\"></div></div></div></div></form><table class=\"table table-condensed\"><thead><tr><th>EP ID</th><th>Project ID</th><th style=width:400px;>Project title</th><th colspan=2>Existing status</th><th colspan=2>New status</th><th>Updated at</th><th>Result</th></tr></thead><tbody><tr data-ng-repeat=\"ep in eiaspermits\" ng-class=\"{danger: ep.error, warning: ep.nochange, success: ep.changed}\"><td>{{ep.eiapermit_id}}</td><td>{{ep.project_id}}</td><td>{{ep.project_title}}</td><td>{{ep.status_id}}</td><td style=\"white-space: nowrap;\">{{ep.status_description}}</td><td>{{ep.status_id_new}}</td><td style=\"white-space: nowrap;\">{{ep.status_description_new}}</td><td style=\"white-space: nowrap;\">{{ep.eiapermit_updated_at | date:'yyyy-MM-dd HH:mm:ss'}}</td><td style=\"white-space: nowrap;\"><span ng-show=ep.updating class=\"loader pull-right\"></span>{{ep.result}}</td></tr></tbody></table></div>"
  );


  $templateCache.put('partials/externalAudit-pirking.html',
    "<div class=top-header><h1>EA Status Update</h1></div><div class=pirking><p>Currently last ID used for a EA is {{lastId}}.</p><form role=form name=criteriaForm novalidation ng-submit=beginPirking()><div class=row><div class=col-md-2><label for=criteria.from>From ID (inclusive)</label> <input integer id=criteria.from ng-required=true ng-model=criteria.from class=\"form-control input-sm\"></div><div class=col-md-2><label for=criteria.to>To ID (inclusive)</label> <input integer id=criteria.to ng-required=true ng-model=criteria.to class=\"form-control input-sm\"></div><div class=col-md-1><label for=dryrun>Dry run</label> <input type=checkbox class=\"form-control input-sm pull-left checkbox-small\" id=dryrun ng-checked=dryrun ng-click=\"dryrun = !dryrun;\"></div><div class=col-md-7><label for=criteria.begin>&nbsp;</label><br><button type=submit class=\"btn btn-sm btn-primary\" id=criteria.begin ng-disabled=\"criteriaForm.$invalid || working\">Begin Status Update</button></div></div><div class=row><div class=col-md-12><br><span class=\"label label-info\" ng-show=dryrun>Try without saving the status back to the database.</span> <span class=\"label label-danger\" ng-hide=dryrun>This will save any changes back to the database.</span></div></div><div class=row><div class=col-md-12><br><span style=padding:5px; class=\"label label-info\">Progress <span class=badge>{{info.finished()}}/{{EaData.length}}</span></span> <span style=padding:5px; class=\"label label-success\">Changed <span class=badge>{{info.changed}}</span></span> <span style=padding:5px; class=\"label label-warning\">No change <span class=badge>{{info.nochange}}</span></span> <span style=padding:5px; class=\"label label-danger\">Error <span class=badge>{{info.error}}</span></span></div></div><div class=row><br><div class=col-md-12><div class=progress><div class=progress-bar role=progressbar style=\"width: {{info.progress()}}%;\"></div></div></div></div></form><table class=\"table table-condensed\"><thead><tr><th>EA ID</th><th>Project ID</th><th style=width:400px;>Project title</th><th colspan=2>Existing status</th><th colspan=2>New status</th><th>Updated at</th><th>Result</th></tr></thead><tbody><tr data-ng-repeat=\"ea in EaData\" ng-class=\"{danger: ea.error, warning: ea.nochange, success: ea.changed}\"><td>{{ea.externalaudit_id}}</td><td>{{ea.project_id}}</td><td>{{ea.project_title}}</td><td>{{ea.status_id}}</td><td style=\"white-space: nowrap;\">{{ea.status_description}}</td><td>{{ea.status_id_new}}</td><td style=\"white-space: nowrap;\">{{ea.status_description_new}}</td><td style=\"white-space: nowrap;\">{{ea.externalaudit_updated_at | date:'yyyy-MM-dd HH:mm:ss'}}</td><td style=\"white-space: nowrap;\"><span ng-show=ea.updating class=\"loader pull-right\"></span>{{ea.result}}</td></tr></tbody></table></div>"
  );
  ...
  $templateCache.put('partials/projectExternalAudits.html',
    "<div class=project-externalaudits><div class=row><div class=col-md-12><div class=edit-container><div class=\"row header visible-md visible-lg\"><div class=col-md-12><table class=table><thead><tr><th>Id</th><th>Status</th><th>Team leader</th><th>Latest submission date</th><th>Date of inspection</th><th>File</th><th><button type=button class=\"btn btn-xs btn-success pull-right\" ng-disabled=routeParams.externalauditId ng-click=newExternalAudit(); ng-hide=\"!auth.canSave('new')\">New</button></th></tr></thead><tbody class=edit-row ng-repeat=\"ea in data.externalaudits\"><tr class=master ng-class=\"{selected: routeParams.externalauditId==ea.id, notvalid: routeParams.externalauditId==ea.id && parts.externalaudit.form.$invalid}\" ng-click=\"openExternalaudit(ea.id, $event)\"><td>{{ea.id}}</td><td>{{data.getCodeFromValuelist('eastatus', ea.status)}}</td><td>{{data.getCodeFromValuelist('team_leader_eia_permit', ea.user_id)}}</td><td>{{ea.documents[0].date_submitted | date:'d MMM yyyy'}}</td><td>{{ea.date_inspection | date:'d MMM yyyy'}}</td><td align=left><a ng-href={{downloadFileUrl(ea.response_document.id)}} ng-click=stopPropagation($event) ng-show=ea.response_document ng-attr-alt={{ea.response_document.filename}} ng-attr-title={{ea.response_document.filename}}><span class=\"glyphicon glyphicon-file\"></span></a></td><td><div style=min-width:65px;><a class=\"btn btn-xs btn-link pull-right text-right\" ng-click=\"preventClickIfDisabled(routeParams.externalauditId, $event)\" ng-disabled=routeParams.externalauditId ng-show=\"routeParams.externalauditId!=ea.id\" ng-href=#/projects/{{routeParams.projectId}}/externalaudits/{{ea.id}}>{{auth.canSave('new') ? \"Edit\" : \"Open\"}}</a> <a class=\"btn btn-xs btn-link pull-right text-right\" ng-click=\"preventClickIfDisabled(parts.externalaudit.state == SavingStateEnum.Loading, $event)\" ng-disabled=\"parts.externalaudit.state == SavingStateEnum.Loading\" ng-show=\"routeParams.externalauditId==ea.id\" ng-href=#/projects/{{routeParams.projectId}}/externalaudits>Close</a> <span class=\"loader pull-right\" ng-show=\"routeParams.externalauditId==ea.id && parts.externalaudit.state == SavingStateEnum.Loading\"></span></div></td></tr><tr ng-if=shouldShowExternalAudit(ea) class=detail><td colspan=8><form role=form name=parts.externalaudit.form novalidation><div class=row><div class=col-md-4><div class=form-group><label for=teamleader_id>Practitioner team leader</label> <select ui-select2={allowClear:true} ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('teamleader_id')\" id=teamleader_id class=\"form-control input-sm\" ng-model=ea.teamleader_id><option></option><option ng-repeat=\"v in valuelists.teamleader\" ng-show=\"v.is_passive == 'false' || ea.teamleader_id == v.id\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-8><div class=form-group><label for=team_members>Practitioner team members</label> <select ui-select2={allowClear:true} multiple placeholder=\"Click to pick\" ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('practitioner_id')\" id=team_members class=\"form-control input-sm\" ng-model=ea.teammember_ids><option></option><option ng-repeat=\"v in valuelists.teammember\" ng-show=\"v.is_passive == 'false' || ea.teammember_ids.includes(v.id)\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=type>Type of external audit</label> <select ui-select2={allowClear:true} ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('type')\" id=type class=\"form-control input-sm\" ng-model=ea.type><option></option><option ng-repeat=\"v in valuelists.external_audit_type\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=user_id>Team leader</label> <select ui-select2={allowClear:true} ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('user_id')\" id=user_id class=\"form-control input-sm\" ng-model=ea.user_id><option></option><option ng-repeat=\"v in valuelists.team_leader_eia_permit | filter:criteriaMatchOfficer1(ea.user_id)\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false, 'dropdown-inactive-item': (v.is_passive == 1 && v.id != false) }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-3><div class=form-group><label for=users>Officers to handle</label> <select ui-select2 multiple placeholder=\"Click to pick\" ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('personnel')\" id=users class=\"form-control input-sm\" ng-model=ea.user_ids><option ng-repeat=\"v in valuelists.officer_eia_permit | filter:criteriaMatchOfficer(ea.user_ids)\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false, 'dropdown-inactive-item': (v.is_passive == 1 && v.id != false) }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-3><div class=form-group><label for=verification_inspection>Verification inspection?</label> <select ui-select2={allowClear:true} ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('verification_inspection')\" id=verification_inspection class=\"form-control input-sm\" ng-model=ea.verification_inspection><option></option><option ng-repeat=\"v in valuelists.yesno\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-3><div class=form-group><label for=date_inspection>Date of inspection</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_inspection')\" id=date_inspection class=\"form-control input-sm input-date\" ng-model=ea.date_inspection data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_inspection')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=date_response>Date of response</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_response')\" id=date_response class=\"form-control input-sm input-date\" ng-model=ea.date_response data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_response')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div><div class=form-group><label for=response>Response</label> <select ui-select2={allowClear:true} ng-change=saveCurrentExternalAudit(ea) ng-disabled=\"!auth.canSave('response')\" id=response class=\"form-control input-sm\" ng-model=ea.response><option></option><option ng-repeat=\"v in valuelists.yesno\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-3><div class=form-group><label>Response letter</label><div class=uploaded-file ng-show=ea.file_metadata_response_id><a ng-href={{downloadFileUrl(ea.file_metadata_response_id)}} ng-show=ea.response_document>{{ea.response_document.filename}}</a><sero-delete-button class=delete-button text=\"'delete'\" warning=\"'NB! You are now going to delete this document. Do you want to proceed?'\" button-class=\"'delete btn btn-sm btn-link'\" ng-hide=\"!auth.canSave('delete_documentation_response_letter')\" on-yes=deleteResponseDocument()></sero-delete-button></div><div><button class=\"btn btn-sm btn-default\" type=button ngf-select=uploadResponseDocument($files) accept={{fileUploadPattern}} ngf-pattern={{fileUploadNgfPattern}} ng-model=fake_response_document name=response_document ng-hide=ea.file_metadata_response_id ng-disabled=\"!auth.canSave('upload_documentation_response_letter')\" ngf-max-size={{fileUploadMaxSize}}>Select File</button><div class=uploading-file ng-show=showUploadingResponseDocument><div class=file>{{responseDocumentFile.name}}<br><div class=progress><div class=progress-bar role=progressbar aria-valuenow={{responseDocumentFile.progress}} aria-valuemin=0 aria-valuemax=100 style=\"width: {{responseDocumentFile.progress}}%;\">{{responseDocumentFile.progress}}%</div></div></div><div class=errors ng-messages=parts.externalaudit.form.response_document.$error role=alert><div ng-message=pattern>The file type is not supported.</div><div ng-message=serverError>Something went wrong on the server.</div><div ng-message=maxSize>The file is to big.</div></div></div></div></div><div class=form-group><label for=date_deadline_compliance>Date for complete compliance</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_deadline_compliance')\" id=date_deadline_compliance class=\"form-control input-sm input-date\" ng-model=ea.date_deadline_compliance data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_deadline_compliance')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div><div class=col-md-6><div class=form-group><label for=remarks_team_leader>Remarks by Team Leader</label> <textarea ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('remarks_team_leader')\" class=\"form-control input-sm\" rows=2 id=remarks_team_leader ng-model=ea.remarks_team_leader></textarea></div><div class=form-group><label for=review_findings>Review findings (Max. 2000 characters)</label> <textarea ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('review_findings')\" class=\"form-control input-sm\" rows=2 id=review_findings ng-model=ea.review_findings></textarea></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=invoice_fees>Fees Invoiced</label> <input ng-blur=saveCurrentExternalAudit(ea) class=\"form-control input-sm\" ng-readonly=\"!auth.canSave('invoice_fees')\" id=invoice_fees ng-model=ea.invoice_fees></div></div><div class=col-md-3><div class=form-group><label for=date_create_invoice>Date of invoicing</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_create_invoice')\" id=date_create_invoice class=\"form-control input-sm input-date\" ng-model=ea.date_create_invoice data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_create_invoice')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div><div class=inline-err ng-show=\"ea.date_invoice_payment && !ea.date_create_invoice.toString()\">Date should be filled if <b>\"Invoice payment date\"</b> is provided</div></div></div><div class=col-md-3><div class=form-group><label for=date_invoice_payment>Invoice payment date</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_invoice_payment')\" id=date_invoice_payment class=\"form-control input-sm input-date\" ng-model=ea.date_invoice_payment data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_invoice_payment')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div><div class=inline-err ng-show=\"ea.date_invoice_payment.toString() && (ea.date_create_invoice > ea.date_invoice_payment)\">Payment date should be ahead of invoicing date</div></div></div><div class=col-md-3><div class=form-group><label for=date_invoice_receipt_issued>Date of receipt issuing</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentExternalAudit(ea) ng-readonly=\"!auth.canSave('date_invoice_receipt_issued')\" id=date_invoice_receipt_issued class=\"form-control input-sm input-date\" ng-model=ea.date_invoice_receipt_issued data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('date_invoice_receipt_issued')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div><div class=inline-err ng-show=\"ea.date_invoice_receipt_issued.toString() && (ea.date_create_invoice > ea.date_invoice_receipt_issued ||  ea.date_invoice_payment > ea.date_invoice_receipt_issued)\">Reciept issuing date should be ahead of invoicing and payment date</div></div></div></div><div class=row><div class=\"form-group col-md-6\" ng-hide=\"!auth.canSave('recipient_email')\"><label>Recipient Emails (;)</label> <input class=\"form-control input-sm\" ng-blur=saveCurrentEmailContact(ea) id=email_contact ng-model=ea.email_contact> <span ng-show=invalidRecipients class=text-danger>Invalid email format.Separate multiple emails with semicolons (;)</span></div><div class=\"col-md-4 pull-right\"><sero-delete-button class=\"delete-button pull-right\" ng-hide=\"!auth.canSave('delete')\" text=\"'Delete External audit'\" warning=\"'NB! Any documents linked to this External audit will also be deleted. Do you want to proceed?'\" button-class=\"'btn-danger'\" on-yes=deleteExternalAudit()></sero-delete-button></div></div><div class=row><div class=col-md-12><p> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.externalaudit.state == SavingStateEnum.Loaded && !ea.is_new\">Last changed by {{ea.updated_by}}</span> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.externalaudit.state == SavingStateEnum.SavingStarted\">Saving...</span> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.externalaudit.state == SavingStateEnum.SavingFinished\">All changes saved</span> <span class=\"saving-message-inline pull-right color-error\" ng-show=\"parts.externalaudit.state == SavingStateEnum.SavingFailed\">Saving failed</span> <span class=\"saving-message-inline pull-right color-error\" ng-show=\"parts.externalaudit.state == SavingStateEnum.Invalid\">All required fields must be filled out</span></p></div></div></form></td></tr></tbody></table></div></div></div></div></div></div>"
  );


  $templateCache.put('partials/projectExternalAuditsDocuments.html',
    "<div class=project-eiasandpermits><div class=row><div class=col-md-12><div class=edit-container><div class=\"row header visible-md visible-lg\"><div class=col-md-12><table class=table><thead><tr><th>Id</th><th>Date of submission</th><th>Document title</th><th>Document code</th><th>Date sent to Director</th><th>Date sent to team leader from dep.secretary</th><th>Date sent to the officer</th><th>Conclusion (Tor)</th><th>File</th><th><button type=button class=\"btn btn-xs btn-success pull-right\" ng-disabled=routeParams.documentId ng-click=newDocument() ng-hide=\"!auth.canSave('new')\">New</button></th></tr></thead><tbody class=edit-row ng-repeat=\"d in data.documents_ea\"><tr class=master ng-class=\"{selected: routeParams.documentId==d.id, notvalid: routeParams.documentId==d.id && parts.document.form.$invalid}\" ng-click=\"openExternalauditDocument(d.id, $event)\"><td>{{d.id}}</td><td>{{d.date_submitted | date:'d MMM yyyy'}}</td><td>{{d.title}}</td><td>{{d.code}}</td><td>{{d.date_sent_director | date:'d MMM yyyy'}}</td><td>{{d.date_sent_from_dep | date:'d MMM yyyy'}}</td><td>{{d.date_sent_officer | date:'d MMM yyyy'}}</td><td>{{data.getCodeFromValuelist('documentconclusion', d.conclusion)}}</td><td align=left><a ng-href={{downloadFileUrl(d.file_metadata_id)}} ng-click=stopPropagation($event) ng-show=d.attachment ng-attr-alt={{d.attachment.filename}} ng-attr-title={{d.attachment.filename}}><span class=\"glyphicon glyphicon-file\"></span></a></td><td><div style=min-width:65px;><a class=\"btn btn-xs btn-link pull-right text-right\" ng-click=\"preventClickIfDisabled(routeParams.documentId, $event)\" ng-disabled=routeParams.documentId ng-show=\"routeParams.documentId!=d.id\" ng-href=#/projects/{{routeParams.projectId}}/externalaudits/{{routeParams.externalauditId}}/documents/{{d.id}}>{{auth.canSave('new') ? \"Edit\" : \"Open\"}}</a> <a class=\"btn btn-xs btn-link pull-right text-right\" ng-click=\"preventClickIfDisabled(parts.document.state == SavingStateEnum.Loading, $event)\" ng-disabled=\"parts.document.state == SavingStateEnum.Loading\" ng-show=\"routeParams.documentId==d.id\" ng-href=#/projects/{{routeParams.projectId}}/externalaudits/{{routeParams.externalauditId}}/documents>Close</a> <span class=\"loader pull-right\" ng-show=\"routeParams.documentId==d.id && parts.document.state == SavingStateEnum.Loading\"></span></div></td></tr><tr ng-if=shouldShowDocument(d) class=detail><td colspan=10><form role=form name=parts.document.form novalidation><div class=row><div class=col-md-3><div class=form-group><label for=document.date_submitted>Date of submission</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-required=true ng-readonly=\"!auth.canSave('document.date_submitted')\" id=document.date_submitted class=\"form-control input-sm input-date\" ng-model=d.date_submitted datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_submitted')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div><div class=col-md-6><div class=form-group><label for=document.title>Document title</label> <input ng-blur=saveCurrentDocument(d) ng-required=true ng-readonly=\"!auth.canSave('document.title')\" class=\"form-control input-sm\" id=document.title ng-model=d.title></div></div><div class=col-md-3><div class=form-group><label>Document file</label><div class=uploaded-file ng-show=d.file_metadata_id><a ng-href={{downloadFileUrl(d.file_metadata_id)}} ng-show=d.attachment>{{d.attachment.filename}}</a><sero-delete-button class=delete-button text=\"'delete'\" warning=\"'NB! You are now going to delete this document. Do you want to proceed?'\" button-class=\"'delete btn btn-sm btn-link'\" ng-hide=\"!auth.canSave('delete')\" on-yes=deleteAttachment()></sero-delete-button></div><div><button class=\"btn btn-sm btn-default\" type=button ngf-select=uploadAttachment($files) accept={{fileUploadPattern}} ngf-pattern={{fileUploadNgfPattern}} ng-model=fake_attachment name=attachment ng-hide=d.file_metadata_id ng-disabled=\"!auth.canSave('delete')\" ngf-max-size={{fileUploadMaxSize}}>Select File</button><div class=uploading-file ng-show=showUploadingAttachment><div class=file>{{attachmentFile.name}}<br><div class=progress><div class=progress-bar role=progressbar aria-valuenow={{attachmentFile.progress}} aria-valuemin=0 aria-valuemax=100 style=\"width: {{attachmentFile.progress}}%;\">{{attachmentFile.progress}}%</div></div></div><div class=errors ng-messages=parts.document.form.attachment.$error role=alert><div ng-message=pattern>The file type is not supported.</div><div ng-message=serverError>Something went wrong on the server.</div><div ng-message=maxSize>The file is to big.</div></div></div></div></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=document.type>Type of document</label> <select ui-select2={allowClear:true} ng-change=calculateDocumentCode() ng-required=true ng-disabled=\"!auth.canSave('document.type')\" id=document.type class=\"form-control input-sm\" ng-model=d.type><option></option><option ng-repeat=\"v in valuelists.documenttypeexternalaudits\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description2}}</option></select></div></div><div class=col-md-3><div class=form-group><label for=document.number>Listed number</label> <input integer ng-blur=saveCurrentDocument(d) ng-required=true ng-focus=clearError() ng-readonly=\"!auth.canSave('document.number')\" class=\"form-control input-sm\" id=document.number ng-model=d.number ng-change=calculateDocumentCode()></div></div><div class=col-md-3><div class=form-group><label for=document.code>Document code</label> <input ng-readonly=true class=\"form-control input-sm\" id=document.code ng-model=d.code> <span style=color:red ng-show=listNumberAlreadyExists>Document code {{documentCode}} exists</span></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=document.sub_copy_no>No. of copies submitted</label> <input integer ng-blur=saveCurrentDocument(d) ng-required=true ng-readonly=\"!auth.canSave('document.sub_copy_no')\" class=\"form-control input-sm\" id=document.sub_copy_no ng-model=d.sub_copy_no ng-change=calculateNumberOfCopiesOfDocument()></div></div><div class=col-md-3><div class=form-group><label for=document.date_next_appointment>Date of next appointment</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-required=true ng-readonly=\"!auth.canSave('document.date_next_appointment')\" id=document.date_next_appointment class=\"form-control input-sm input-date\" ng-model=d.date_next_appointment data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_next_appointment')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=document.coordinator_copy_no>No. of copies to team leader</label> <input integer ng-blur=saveCurrentDocument(d) ng-readonly=true class=\"form-control input-sm\" id=document.coordinator_copy_no ng-model=d.coordinator_copy_no></div></div><div class=col-md-3><div class=form-group><label for=document.date_copies_coordinator>Date copies sent to team leader</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.date_copies_coordinator')\" id=document.date_copies_coordinator class=\"form-control input-sm input-date\" ng-model=d.date_copies_coordinator data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_copies_coordinator')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=document.director_copy_no>No. of to copies Director</label> <input integer ng-blur=saveCurrentDocument(d) ng-readonly=true class=\"form-control input-sm\" id=document.director_copy_no ng-model=d.director_copy_no></div></div><div class=col-md-3><div class=form-group><label for=document.date_sent_director>Date sent to Director</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.date_sent_director')\" id=document.date_sent_director class=\"form-control input-sm input-date\" ng-model=d.date_sent_director data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_sent_director')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div><div class=inline-err ng-show=\"(d.date_copies_coordinator.toString() && d.date_sent_director.toString()) && d.date_copies_coordinator > d.date_sent_director\">Document should be sent to team leaders before the director</div></div></div><div class=col-md-3><div class=form-group><label for=document.date_sent_from_dep>Date sent to team leader from dep.</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.date_sent_from_dep')\" id=document.date_sent_from_dep class=\"form-control input-sm input-date\" ng-model=d.date_sent_from_dep data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_sent_from_dep')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div><div class=col-md-3><div class=form-group><label for=document.date_sent_officer>Date sent to the officer</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.date_sent_officer')\" id=document.date_sent_officer class=\"form-control input-sm input-date\" ng-model=d.date_sent_officer data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_sent_officer')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=document.conclusion>Conclusion (Tor)</label> <select ui-select2={allowClear:true} ng-change=saveCurrentDocument(d); ng-disabled=\"!auth.canSave('document.conclusion') || isDisabledBasedOnRule('document.conclusion')\" id=document.conclusion class=\"form-control input-sm\" ng-model=d.conclusion><option></option><option ng-repeat=\"v in valuelists.documentconclusion\" value={{v.id}} ng-class=\"{'dropdown-partition-bar': v.id == false }\" ng-disabled=\"v.id == false\">{{v.description1}}</option></select></div></div><div class=col-md-3><div class=form-group><label for=document.date_sent_officer>Date of conclusion for TOR</label><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-disabled=\"isDisabledBasedOnRule('document.type')\" ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.date_conclusion')\" id=document.date_conclusion class=\"form-control input-sm input-date\" ng-model=d.date_conclusion data-datepicker-popup=\"d MMM yyyy\" data-is-open=opened data-datepicker-options=datepickerOptions data-show-button-bar=false> <span class=input-group-btn><button type=button ng-disabled=\"!auth.canSave('document.date_conclusion') || isDisabledBasedOnRule('document.type')\" class=\"btn btn-default btn-sm\" ng-click=open($event)><span class=\"glyphicon glyphicon-calendar\"></span></button></span></div></div></div><div class=col-md-3><div class=form-group><label>Response document</label><div class=uploaded-file ng-show=d.file_metadata_response_id><a ng-href={{downloadFileUrl(d.file_metadata_response_id)}} ng-show=d.response_document>{{d.response_document.filename}}</a><sero-delete-button class=delete-button text=\"'delete'\" warning=\"'NB! You are now going to delete this document. Do you want to proceed?'\" button-class=\"'delete btn btn-sm btn-link'\" ng-hide=\"!auth.canSave('document.file_metadata_response_id')\" on-yes=deleteResponseDocument()></sero-delete-button></div><div><button class=\"btn btn-sm btn-default\" type=button ngf-select=uploadResponseDocument($files) accept={{fileUploadPattern}} ngf-pattern={{fileUploadNgfPattern}} ng-model=fake_response_document name=response_document ng-hide=d.file_metadata_response_id ng-disabled=\"!auth.canSave('document.file_metadata_response_id')\" ngf-max-size={{fileUploadMaxSize}}>Select File</button><div class=uploading-file ng-show=showUploadingResponseDocument><div class=file>{{responseDocumentFile.name}}<br><div class=progress><div class=progress-bar role=progressbar aria-valuenow={{responseDocumentFile.progress}} aria-valuemin=0 aria-valuemax=100 style=\"width: {{responseDocumentFile.progress}}%;\">{{responseDocumentFile.progress}}%</div></div></div><div class=errors ng-messages=parts.document.form.response_document.$error role=alert><div ng-message=pattern>The file type is not supported.</div><div ng-message=serverError>Something went wrong on the server.</div><div ng-message=maxSize>The file is to big.</div></div></div></div></div></div><div class=col-md-3><div class=form-group><label for=document.folio_no>File and foliono.</label> <input ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.folio_no')\" class=\"form-control input-sm\" id=document.folio_no ng-model=d.folio_no></div></div></div><div class=row><div class=col-md-6><div class=form-group><label for=document.remarks>Remarks</label> <textarea ng-blur=saveCurrentDocument(d) ng-readonly=\"!auth.canSave('document.remarks')\" class=\"form-control input-sm\" rows=2 id=document.remarks ng-model=d.remarks></textarea></div></div></div></form><div class=row><div class=col-md-12><sero-delete-button class=\"delete-button pull-right\" ng-hide=\"!auth.canSave('delete')\" text=\"'Delete document'\" warning=\"'Do you really want to delete this record?'\" button-class=\"'btn-danger'\" on-yes=deleteDocument()></sero-delete-button><button ng-hide=\"!auth.canSave('can_email')\" ng-click=\"createEmailOrder('EA', ea.id, d.id, ea)\" ng-disabled=\"!ea.email_contact || d.email_order\" class=\"btn btn-success pull-right\" style=margin-top:35px;margin-right:10px>{{(d.id)?getEmailerObj(d).btnMsg : 'Send email'}}</button><div class=\"btn-group move-button pull-right\" dropdown auto-close=disabled is-open=moveButton.isopen ng-hide=\"!auth.canSave('move') || d.is_new\"><button type=button class=\"btn btn-warning pull-right\" dropdown-toggle ng-hide=\"!auth.canSave('new')\">Move document<span class=caret></span></button><ul class=dropdown-menu role=menu><li role=menuitem><form role=form name=moveButton.form novalidation><div class=row><div class=col-md-8><label for=moveButton.id>EA ID</label> <input integer id=moveButton.id ng-required=true ng-model=moveButton.id class=\"form-control input-sm\"></div><div class=col-md-4><label for=moveButton.move>&nbsp;</label><br><button type=button class=\"btn btn-sm btn-primary use-button pull-right\" id=moveButton.move ng-disabled=moveButton.form.$invalid ng-click=moveDocument()>Move</button></div></div><div class=row ng-show=moveButton.error><div class=col-md-12>{{moveButton.error}}</div></div></form></li></ul></div></div><span ng-show=\"ea.email_order.order_status == 4\" class=\"text-danger pull-right\">Contact an admin to fix issue with email services </span><span ng-show=failedToSendMail class=\"text-danger pull-right\">We could not send the email because you haven’t added the email address or another mandatory information is missing</span></div><div class=row><div class=col-md-12><p> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.document.state == SavingStateEnum.Loaded && !d.is_new\">Last changed by {{d.updated_by}}</span> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.document.state == SavingStateEnum.SavingStarted\">Saving...</span> <span class=\"saving-message-inline pull-right\" ng-show=\"parts.document.state == SavingStateEnum.SavingFinished\">All changes saved</span> <span class=\"saving-message-inline pull-right color-error\" ng-show=\"parts.document.state == SavingStateEnum.SavingFailed\">Saving failed</span> <span class=\"saving-message-inline pull-right color-error\" ng-show=\"parts.document.state == SavingStateEnum.Invalid\">All required fields must be filled out</span></p></div></div></td></tr></tbody></table></div></div></div></div></div></div>"
  );
  ...
  $templateCache.put('partials/publicPractitioners.html',
    "<div class=top-header><p class=\"text-center lead\">LIST OF CERTIFIED AND REGISTERED ENVIRONMENTAL PRACTITIONERS IN UGANDA</p><div class=row><div class=col-lg-6><p>The Committee of Environmental Practitioners of Uganda, in exercise of the powers conferred upon it by the National Environment (Conduct and Certification of Environmental Practitioners) Regulations, 2003, has Certified and Registered the following listed persons and partnerships as Environmental Practitioners for purposes of carrying out Environmental Impact Assessments (EIA), Environmental Audits (EA) and/or to coordinate the conduct of EIA and/or EA respectively, in Uganda.<p>In accordance with Regulation (32) 2, of the National Environment (Conduct and Certification of Environmental Practitioners) Regulations, 2003, an Environmental Practitioner’s registration is valid from the date of registration until the 31st day of December of the last registration year. Notwithstanding the aforementioned, a registered Environmental Practitioner may continue to practice up to the 28th February the following year, pending renewal of the registration.</p></p></div><div class=col-lg-6><b>The Practitioners have been registered under the following categories:</b><table style=width:100%><tr><td>&nbsp;ENVIRONMENTAL IMPACT ASSESSORS</td><td>&nbsp;<span class=\"label label-info\">EIA - TM</span></td></tr><tr><td>&nbsp;ENVIRONMENTAL AUDITORS</td><td>&nbsp;<span class=\"label label-success\">EA - TM</span></td></tr><tr><td>&nbsp;FOREIGN PRACTITIONERS</td><td>&nbsp;<span class=\"label label-danger\">FP</span></td></tr><tr><td>&nbsp;ENVIRONMENTAL PARTNERSHIPS (EIA)</td><td>&nbsp;<span class=\"label label-warning\">EP - EIA</span></td></tr><tr><td>&nbsp;ENVIRONMENTAL PARTNERSHIPS (EA)</td><td>&nbsp;<span class=\"label label-warning\">EP - EA</span></td></tr><tr><td>&nbsp;TEAM LEADERS (EIA)</td><td>&nbsp;<span class=\"label label-info\">EIA - TL</span></td></tr><tr><td>&nbsp;TEAM LEADERS (EA)</td><td>&nbsp;<span class=\"label label-success\">EA - TL</span></td></tr></table></div></div></div><div class=practitioners><div class=row><div class=col-md-8><form class=form-inline role=form><div class=form-group><label class=sr-only for=filterText>Filter</label> <input class=\"form-control input-filter\" id=filterText placeholder=\"Filter (try anything, e.g. name, city, eia or audit)\" ng-model=filterText></div><span>Total: {{ (practitioners | filter: filterText).length }}</span></form></div><div class=col-md-4><p class=\"alert-warning text text-center\">When you click on a line, additional information will be shown.<br></p></div><div class=col-md-12><hr></div></div><div class=\"row header visible-md visible-lg\"><div class=col-md-3>Practitioner</div><div class=col-md-3>Organisation name</div><div class=col-md-3>Address</div><div class=col-md-1>City</div><div class=col-md-2>Certificates</div></div><div class=\"row practitioner\" ng-repeat=\"p in practitioners | filter:filterText\"><div class=col-md-12><form role=form name=form novalidation><div class=\"row master\" ng-click=toggleRow(p); ng-class=\"{selected: current==p, notvalid: form.$invalid}\"><div class=col-md-3 title=\"id={{ p.id }}\">{{ p.person }}</div><div class=col-md-3>{{ p.organisation_name }}</div><div class=\"col-md-3 hidden-xs\">{{ p.visiting_address }}</div><div class=\"col-md-1 hidden-xs\">{{ p.city }}</div><div class=col-md-2><span class=\"label certificate-label\" ng-class=\"hasAuditTL(p) ? 'label-success' :'label-default noCertificate'\">EA-TL</span> <span class=\"label certificate-label\" ng-class=\"hasAuditTM(p) ? 'label-success' :'label-default noCertificate'\">EA-TM</span> <span class=\"label certificate-label\" ng-class=\"hasEiaTL(p) ? 'label-info' :'label-default noCertificate'\">EIA-TL</span> <span class=\"label certificate-label\" ng-class=\"hasEiaTM(p) ? 'label-info' :'label-default noCertificate'\">EIA-TM</span> <span class=\"label certificate-label\" ng-class=\"hasPartnershipEia(p) ? 'label-warning' :'label-default noCertificate'\">EP-EIA</span> <span class=\"label certificate-label\" ng-class=\"hasPartnershipEa(p) ? 'label-warning' :'label-default noCertificate'\">EP-EA</span> <span class=\"label certificate-label\" ng-class=\"hasForeignPractitioner(p) ? 'label-danger' :'label-default noCertificate'\">FP</span> <button type=button class=\"btn btn-xs btn-link pull-right text-right\" hover-on-parent=.master ng-show=\"current!=p\">{{ canSave() ? \"Edit\" : \"Open\" }}</button> <button type=button class=\"btn btn-xs btn-link pull-right text-right\" hover-on-parent=.master ng-show=\"current==p\">Close</button> <span class=\"loader pull-right\" ng-show=\"loading==p\"></span></div></div><div class=\"row detail\" ng-if=\"current==p\" ng-init=setCurrentForm(form)><div class=col-md-12><div class=row><div class=col-md-2><div class=form-group><label for=person>Title</label> <input ng-readonly=!canSave() id=p_title ng-required=true class=\"form-control input-sm\" ng-model=\"find(valuelists.practitioner_title, 'id', p.practitioner_title_id).description1\"></div></div><div class=col-md-3><div class=form-group><label for=person>Practitioner</label> <input ng-readonly=!canSave() ng-required=true class=\"form-control input-sm\" id=person ng-model=p.person></div></div><div class=col-md-2><div class=form-group><label for=tin>TIN</label> <input ng-readonly=!canSave() integer max=9999999999 class=\"form-control input-sm\" id=tin ng-model=p.tin></div></div><div class=col-md-5><div class=form-group><label for=organisation_name>Organisation Name</label> <input ng-readonly=!canSave() ng-required=false class=\"form-control input-sm\" id=organisation_name ng-model=p.organisation_name></div></div></div><div class=row><div class=col-md-6><div class=form-group><label for=visiting_address>Address</label> <input ng-readonly=!canSave() class=\"form-control input-sm\" id=visiting_address ng-model=p.visiting_address></div></div><div class=col-md-2><div class=form-group><label for=box_no>P.O. Box</label> <input ng-readonly=!canSave() integer ng-required=true class=\"form-control input-sm\" id=box_no ng-model=p.box_no></div></div><div class=col-md-4><div class=form-group><label for=city>City</label> <input ng-readonly=!canSave() ng-required=true class=\"form-control input-sm\" id=city ng-model=p.city></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=phone>Phone</label> <input ng-readonly=!canSave() class=\"form-control input-sm\" id=phone ng-model=p.phone></div></div><div class=col-md-3><div class=form-group><label for=fax>Fax</label> <input ng-readonly=!canSave() class=\"form-control input-sm\" id=fax ng-model=p.fax></div></div><div class=col-md-6><div class=form-group><label for=email>E-mail</label> <input ng-readonly=!canSave() class=\"form-control input-sm\" id=email ng-model=p.email></div></div></div><div class=row><div class=col-md-4><div class=form-group><label for=qualifications>Qualifications</label> <textarea ng-readonly=!canSave() ng-required=true class=\"form-control input-sm\" rows=3 id=qualifications ng-model=p.qualifications></textarea></div></div><div class=col-md-4><div class=form-group><label for=expertise>Expertise</label> <textarea ng-readonly=!canSave() ng-required=true class=\"form-control input-sm\" rows=3 id=expertise ng-model=p.expertise></textarea></div></div><div class=col-md-4><div class=form-group><label for=email>Remarks</label> <textarea ng-readonly=!canSave() class=\"form-control input-sm\" rows=3 id=remarks ng-model=p.remarks></textarea></div></div></div><div class=row><div class=col-md-8><h4>Certificates</h4></div><div class=col-md-4><sero-delete-button class=\"delete-button pull-right\" text=\" p.is_new ? 'Cancel new practitioner' : 'Delete practitioner'\" warning=\"'NB! You are now going to delete this practitioner. Do you want to proceed?'\" button-class=\"'btn btn-danger btn-sm pull-right'\" ng-hide=!canSave() on-yes=\"deletePractitioner($index, p)\"></sero-delete-button></div></div><div class=\"row certificates\"><div class=col-md-12><table class=table><thead><tr><th>Year</th><th>Date of entry</th><th>Type of certificate</th><th>Number</th><th>Certificate number</th><th>Conditions</th><th>Cancelled?</th><th>Remarks</th><th></th></tr></thead><tbody><tr ng-repeat=\"c in p.practitioner_certificates\" ng-hide=c.is_deleted><td><input ng-readonly=!canSave() ng-required=true ng-change=calculateCertificateNumber(c) integer min=\"{{ certificateYearMin }}\" max=\"{{ certificateYearMax }}\" class=\"form-control input-sm input-year\" id=c_year ng-model=c.year></td><td><div class=\"input-group container-datepicker\" ng-controller=DatePickerController><input ng-readonly=!canSave() class=\"form-control input-sm input-date\" ng-model=c.date_of_entry data-datepicker-popup=\"d MMM yyyy\" data-show-button-bar=false> <span class=input-group-btn></span></div></td><td><input ng-readonly=!canSave() id=c_cert_type ng-required=true class=\"form-control input-sm\" ng-model=\"find(valuelists.practitionertype, 'id', c.cert_type).description1\"></td><td><input ng-readonly=!canSave() id=c_number ng-change=calculateCertificateNumber(c) ng-required=true class=\"form-control input-sm\" ng-model=c.number></td><td><input ng-readonly=true id=c_cert_no certificate-number ng-required=true class=\"form-control input-sm\" ng-model=c.cert_no></td><td><input ng-readonly=!canSave() id=c_conditions ng-required=true class=\"form-control input-sm\" ng-model=\"find(valuelists.practitionermembertype, 'id', c.conditions).description2\"></td><td><input type=checkbox ng-disabled=!canSave() class=\"checkbox-small form-control input-sm\" id=c_is_cancelled ng-model=fake ng-checked=c.is_cancelled ng-click=\"c.is_cancelled = !c.is_cancelled\"></td><td><input ng-readonly=!canSave() class=\"form-control input-sm input-remarks warningfont\" id=c_remarks ng-model=c.remarks></td><td></td></tr></tbody></table></div></div></div></div></form></div></div><br></div><style>table, th, td {\n" +

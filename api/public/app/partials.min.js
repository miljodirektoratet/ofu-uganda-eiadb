angular.module('seroApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('partials/searchTabs.html',
    "<div class=search><ul class=\"nav nav-tabs\" role=tablist><li ng-class=\"{active: tab==SearchTabEnum.Projects}\" ng-show=userinfo.info.features.notproduction><a ng-href=#/search/projects>Project</a></li><li ng-class=\"{active: tab==SearchTabEnum.AuditsInspections}\"><a href=#/search/auditsinspections>Audit and Inspection</a></li></ul><div ng-if=\"tab==SearchTabEnum.Projects\">NOT IMPLEMENTED</div><div ng-if=\"tab==SearchTabEnum.EiasPermits\">NOT IMPLEMENTED</div><div ng-if=\"tab==SearchTabEnum.AuditsInspections\"><div ng-controller=SearchAuditsInspectionsController ng-include=\"'partials/searchAuditsInspections.html'\"></div></div></div>"
  );
  ...
  $templateCache.put('partials/statisticsTablePanel.html',
    "<div class=\"panel panel-default\"><div class=panel-heading><h2 class=panel-title>{{part.title}}</h2></div><div class=panel-body><table class=\"table table-condensed\"><thead><tr><th>{{part.label1}}</th><th><div class=text-right>{{part.label2}}</div></th></tr></thead><tbody><tr ng-repeat=\"row in part.rows\"><td title={{row.id}}>{{row.description}}</td><td><div class=text-right>{{row.count}}</div></td></tr></tbody></table></div></div>"
  );

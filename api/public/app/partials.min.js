angular.module('seroApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('partials/searchAuditsInspections.html',
    "<div class=search-auditsandinspections><div class=row><div class=col-md-12><form ng-submit=setSearchUrl()><div class=row><div class=col-md-3><div class=form-group><label for=project_title>Project title</label><input class=\"form-control input-sm\" id=project_title ng-model=criteria.project_title set-focus></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=auditinspection_year>Year</label><input class=\"form-control input-sm\" id=auditinspection_year ng-model=\"criteria.auditinspection_year\"></div></div></div><div class=row><div class=col-md-3><div class=form-group><label for=auditinspection_action_taken>Action taken</label><select ui-select2 id=auditinspection_action_taken class=\"form-control input-sm\" ng-model=criteria.auditinspection_action_taken><option ng-repeat=\"v in valuelists.actiontaken\" value={{v.id}}>{{v.description1}}</option></select></div></div></div><div class=row><div class=col-md-12><button type=submit class=\"btn btn-primary\">Search</button> <button type=button class=\"btn btn-link\" ng-click=reset()>Reset search criteria</button></div></div></form></div></div><div class=row><div class=col-md-12><hr></div></div><div class=row ng-show=isSearching><div class=col-md-12><div class=\"loader-standalone center-block\"></div></div></div><div class=row ng-show=showResultGrid><div class=col-md-12><div ui-grid=gridOptions ui-grid-resize-columns ui-grid-move-columns class=grid></div></div></div></div>"
  );

...
for row in dataSheet.rows[10:]:   
  columnValues = [column.value for column in row[:14]]
  if not row[1].value: continue # Ignore row if missing column name
  
  order, fieldname, datatype, visible, caption, uitab, fieldtype, valuelist, mandatory, inputtype, role, tablename, tabletype, comments = list(columnValues)
  if comments == "Delete this column":
    continue
...
for tablename in tables:
  ...
  if "MISSING" in schemaUp:
    print(schemaUp)
    exit()    
  if onlyTheese and tablename not in onlyTheese:
    print("Ignore (in prodcution): ", tablename)
    continue
